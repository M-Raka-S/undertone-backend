...
beforeEach(function () {
    ...
    $this->sortedUsers = $this->users->sortBy('id')->values();
...
test('success when reading all users', function () {
    ...
    $response->assertJsonPath('data.*.username', $this->sortedUsers->slice(0, 15)->pluck('username')->toArray());
    $response = $this->get('/api/users/page/2');
    $response->assertStatus(200);
    $response->assertJsonPath('data.*.username', $this->sortedUsers->slice(15, 15)->pluck('username')->toArray());
});
